...
public class OperandParser {
	...
	private Command parseF3m(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		// check if literal
		if (parser.advanceIf('=')) {
			InstructionF34Base cmd = new InstructionF3m(loc, label, mnemonic, new Flags(Flags.SIMPLE, Flags.NONE), 0,
					null);
		...
		if (Character.isDigit(parser.peek()) || parser.peek() == '-') {
			...
			symbol = null;
		} else if (Character.isLetter(parser.peek()) || parser.peek() == '$') {
			operand = 0;
	...
	private Command parseF4m(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		// check if literal
		if (parser.advanceIf('=')) {
			InstructionF34Base cmd = new InstructionF4m(loc, label, mnemonic, new Flags(Flags.SIMPLE, Flags.NONE), 0,
					null);
			StorageData lit = parseLiteralData();
			return new InstructionLiteral(cmd, lit);
		}
	...
	private Command parseD(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		case Opcode.CSECT:
			return new DirectiveCSECT(loc, label, mnemonic);
		case Opcode.LTORG:
			return new DirectiveLTORG(loc, label, mnemonic);
		case Opcode.NOBASE:
			return new DirectiveNOBASE(loc, label, mnemonic);
		}
		return null;
	}
	...
	private Command parseDe(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		switch (mnemonic.opcode) {
		case Opcode.BASE:
			return new DirectiveBASE(loc, label, mnemonic, expr);
		case Opcode.START:
			return new DirectiveSTART(loc, label, mnemonic, expr);
		case Opcode.END:
			return new DirectiveEND(loc, label, mnemonic, expr);
		case Opcode.EQU:
			return new DirectiveEQU(loc, label, mnemonic, expr);
		}
		return null;
	}
	...
	private Command parseDs_(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		switch (mnemonic.opcode) {
		case Opcode.EXTDEF:
			return new DirectiveEXTDEF(loc, label, mnemonic, names);
		case Opcode.EXTREF:
			return new DirectiveEXTREF(loc, label, mnemonic, names);
		}
		return null;
	}
	...
	public Command parse(Location loc, String label, Mnemonic mnemonic) throws AsmError {
		...
		case F1:
			return parseF1(loc, label, mnemonic);
		case F2n:
			return parseF2n(loc, label, mnemonic);
		case F2r:
			return parseF2r(loc, label, mnemonic);
		case F2rn:
			return parseF2rn(loc, label, mnemonic);
		case F2rr:
			return parseF2rr(loc, label, mnemonic);
		case F3:
			return parseF3(loc, label, mnemonic);
		case F3m:
			return parseF3m(loc, label, mnemonic);
		case F4m:
			return parseF4m(loc, label, mnemonic);
		case D:
			return parseD(loc, label, mnemonic);
		case De:
			return parseDe(loc, label, mnemonic);
		case De0:
			return parseDe0(loc, label, mnemonic);
		case Ds0:
			return parseDs0(loc, label, mnemonic);
		case Ds_:
			return parseDs_(loc, label, mnemonic);
		case Se:
			return parseSe(loc, label, mnemonic);
		case Sd:
			return parseSd(loc, label, mnemonic);
		case Sd_:
			return parseSd_(loc, label, mnemonic);
		}
		return null;
	}
