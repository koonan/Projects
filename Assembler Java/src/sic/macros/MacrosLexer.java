...
public class MacrosLexer extends Input {
	...
	public boolean isAlpha() {
		return Character.isLetter(peek()) || peek() == '_' || peek() == '$' || peek() == '&';
	}

	public boolean isAlphanumeric() {
		return Character.isLetterOrDigit(peek()) || peek() == '_' || peek() == '$' || peek() == '&';
	}

	public boolean isAlphanumericOperand() {
		String pattern = "" + peek();
		return Character.isLetterOrDigit(peek())
				|| (pattern.matches("\\S+|\\s+") && !pattern.matches("\\n") && pos < buf.length());

	}

	public String readAlphanumericOperand() {
		int mark = pos;
		skipAlphanumericOperand();
		return extract(mark);
	}

	public void skipAlphanumericOperand() {
		while (isAlphanumericOperand())
			advance();
	}
	...
	public String readDigits(int radix) {
		int mark = pos;
		while (Character.digit(peek(), radix) != -1)
			advance();
		return extract(mark);
	}
	...
	public String readIfOperand() {
		if (isAlphanumericOperand())
			return readAlphanumericOperand();
		return null;
	}
	...
	/**
	 * Parse integer in any of the available formats. Formats: standard, 0bBIN,
	 * 0oOCT, 0xHEX. If minus sign is present it must be immediately followed by
	 * the number.
	 * 
	 * @param lo
	 *            ... lower limit
